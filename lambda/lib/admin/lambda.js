...
function addPermissions(lambda, resource) {
  if (permissions.hasOwnProperty(resource)) {
    var awsAddPermission = Bluebird.promisify(lambda.addPermission, { context: lambda });

    return Bluebird.each(permissions[resource], function(permission) {
      return Bluebird.resolve()
        .then(function() {
            return awsAddPermission(permission);
          })
          .then(function() {
            grunt.log.writeln('Permission for resource ' + resource +
              ' was added for SourceArn ' + permission.SourceArn);
          })
          .catch(function(err) {
  ...
  Bluebird.try(function() {
    ...
    .then(function(role) {
      ...
      return Bluebird.resolve()
        ...
        .then(function() {
            return addPermissions(lambda, resource);
          })
        .then(function() {
            done();
          })
