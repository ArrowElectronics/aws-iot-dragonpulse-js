...
ARROW_DIR="arrow"
ARROW_APPLICATION="aws-iot-dragonpulse-js"
ARROW_APP_SEARCH_NEEDLE="DragonPulse"
ARROW_APP_NAME="dragonpulse"
ARROW_CERT_DIR=""
...
#------------------
	...
	EXT_INPUT=$(aws iam list-roles --query 'Roles[?RoleName.contains(@, `DragonPulse-ApiGateway`)].RoleName' --output text)
...
#------------------
	...
	#TODO (gtam): find a way to insert shell var into aws 
	IDENTIFIER_INPUT=$(aws apigateway get-rest-apis --query 'items[?name.contains(@, `DragonPulse`)].id' --output text)
